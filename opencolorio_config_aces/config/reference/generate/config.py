...
DESCRIPTION_ACES_TRANSFORM_ID = "ACEStransformID: {}"
"""
Template for the description of an *ACEStransformID*.

DESCRIPTION_ACES_TRANSFORM_ID : unicode
"""
...
def ctl_transform_to_description(
    ...
    *ACES* *CTL* transform.
    ...
    amf_relations : mapping, optional
        *ACES* *AMF* relations used to extend the *ACES* *CTL* transform
        description.
    ...
    if describe != DescriptionStyle.NONE:
        ...
        if describe in (
            ...
            if describe in (
                ...
                description.append(
                    DESCRIPTION_ACES_TRANSFORM_ID.format(aces_transform_id)
                )
            else:
            ...
            description.extend(
                [
                    DESCRIPTION_ACES_TRANSFORM_ID.format(amf_aces_transform_id)
                    for amf_aces_transform_id in amf_relations.get(
                        aces_transform_id, []
                    )
                ]
            )
    ...
    *CTL* transform.
    ...
    describe : bool, optional
        *ACES* *CTL* transform description style. Any value from the
        :class:`opencolorio_config_aces.DescriptionStyle` enum.
    amf_relations : mapping, optional
       *ACES* *AMF* relations used to extend the *OpenColorIO* `Colorspace`
        description.
    analytical : bool, optional
        Whether to generate the *OpenColorIO* transform family that
        analytically matches the given *ACES* *CTL* transform, i.e. true to
    ...
    description = ctl_transform_to_description(
        ctl_transform, describe, amf_relations, colorspace_factory, **kwargs
    )
    ...
    **kwargs,
    ...
    describe : bool, optional
        *ACES* *CTL* transform description style. Any value from the
        :class:`opencolorio_config_aces.DescriptionStyle` enum.
    amf_relations : mapping, optional
        *ACES* *AMF* relations used to extend the *OpenColorIO* `Look`
        description.
    analytical : bool, optional
        Whether to generate the *OpenColorIO* transform family that
        analytically matches the given *ACES* *CTL* transform, i.e. true to
    ...
    family = ctl_transform_to_transform_family(ctl_transform, analytical)
    description = ctl_transform_to_description(
        ctl_transform, describe, amf_relations, look_factory, **kwargs
    )
...
def style_to_view_transform(
    ...
    **kwargs,
    ...
    describe : int, optional
        ...
        enum.
    amf_relations : mapping, optional
        *ACES* *AMF* relations used to extend the *OpenColorIO* `ViewTransform`
        description.
    signature_only : bool, optional
    ...
    if describe != DescriptionStyle.NONE:
        ...
        if describe in (
            ...
            for aces_transform_id in aces_transform_ids:
                description.extend(
                    [
                        DESCRIPTION_ACES_TRANSFORM_ID.format(
                            amf_aces_transform_id
                        )
                        for amf_aces_transform_id in amf_relations.get(
                            aces_transform_id, []
                        )
...
def style_to_display_colorspace(
    ...
    **kwargs,
    ...
    describe : int, optional
        ...
        enum.
    amf_relations : mapping, optional
        *ACES* *AMF* relations used to extend the *OpenColorIO* display
    ...
    if describe != DescriptionStyle.NONE:
        ...
        description.extend(
            [
                DESCRIPTION_ACES_TRANSFORM_ID.format(amf_aces_transform_id)
                for amf_aces_transform_id in amf_relations.get(style, [])
            ]
        )
...
def generate_config_aces(
    ...
    *Reference Config - Mapping* sheet from a
    ...
    amf_relations = generate_amf_relations(ctl_transforms)
